...
func TestOpenAIGatewayService_SelectAccountByPreviousResponseID_DBRuntimeRecheckTransportMismatchMiss(t *testing.T) {
	ctx := context.Background()
	groupID := int64(25)
	staleAccount := &Account{
		ID:          14,
	...
	require.NoError(t, store.BindResponseAccount(ctx, groupID, "resp_prev_db_transport", dbAccount.ID, time.Hour))

	selection, err := svc.SelectAccountByPreviousResponseID(ctx, &groupID, "resp_prev_db_transport", "gpt-5.1", nil, false)
	require.NoError(t, err)
	require.Nil(t, selection, "DB 中已回退为 HTTP 的账号不应继续命中 previous_response_id 粘连")
	boundAccountID, getErr := store.GetResponseAccount(ctx, groupID, "resp_prev_db_transport")
	require.NoError(t, getErr)
	require.Zero(t, boundAccountID)
}
...
func TestOpenAIGatewayService_SelectAccountByPreviousResponseID_CompactUnsupportedClearsSticky(t *testing.T) {
	ctx := context.Background()
	groupID := int64(25)
	account := Account{
		ID:          31,
	...
	require.NoError(t, store.BindResponseAccount(ctx, groupID, "resp_prev_compact_unsupported", account.ID, time.Hour))

	selection, err := svc.SelectAccountByPreviousResponseID(ctx, &groupID, "resp_prev_compact_unsupported", "gpt-5.4", nil, true)
	require.NoError(t, err)
	require.Nil(t, selection, "explicitly unsupported compact account should not be reused")

	boundAccountID, getErr := store.GetResponseAccount(ctx, groupID, "resp_prev_compact_unsupported")
	require.NoError(t, getErr)
